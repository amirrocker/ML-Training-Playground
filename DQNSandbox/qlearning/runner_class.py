...
class SessionProcessor:
    ...
    def runSessions(self, trainingSession, connection):
        ...
        rx.from_list(trainingSession.trainings).pipe(
            ops.map(lambda s: run_training(s, connection)),
            ops.subscribe_on(pool_scheduler)
        ).subscribe(
            on_next=lambda s: print("Process 1: {0} {1}".format(current_thread().name, s)),
            on_error=lambda e: print("Error e: {0}".format(e)),
...
def run_training(value, connection, path):  # TODO rename this asap

    # doProcessing(connection)
    # PATH = 'C:/Users/info/Documents/Development/PycharmProjects/SelfDeepQLearning/DQNSandbox/qlearning'

    '''
    assert len(path) != 0, 'invalid path argument in run_training'
    print("path: %s" % path)
    model = ModelFileExecutor(path)
    model.execute('TestDynamicExec.py')
